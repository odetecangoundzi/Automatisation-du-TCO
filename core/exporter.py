...
def _detect_companies(df):
    """Détecte les noms d'entreprises à partir des colonnes."""
    companies = []
    seen = set()
...
def _get_row_style(row_type):
    """Retourne (font, fill) selon le type de ligne."""
    styles = {
    ...
    return styles.get(row_type, (FONT_DATA, None))
